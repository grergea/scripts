...
def generate_signed_url(mode, scheme, host, path, key, start_time, hex_time=False, hash_type='md5', utv_time_offset=3600, utv_time_param='px-time', utv_hash_param='px-hash', uid=None, uid_type='mac', sign_order='path-key-time', verbose=False):
    if hex_time:
    ...
    if verbose:
        print(f"[DEBUG] Mode: {mode}, Scheme: {scheme}, Host: {host}, Path: {path}, Key: {mask_key(key)}, Time: {time_str}, Hash: {hash_type}")

    if mode == 'A':
        signed_key = generate_signature(build_sign_string(path, key, time_str, sign_order), hash_type, key, verbose)
        ...
        rand = ''.join(random.choices('0123456789', k=9))  # Generate a random 9-digit string
        ...
        if verbose:
            print(f"[DEBUG] UTV mode: original time {time_str} -> adjusted time {utv_time} (+{utv_time_offset} seconds)")
            print(f"[DEBUG] UTV parameters: time_param='{utv_time_param}', hash_param='{utv_hash_param}'")
...
def main():
    ...
    parser.add_argument('--hash', choices=['md5', 'sha256'], default='md5', help='Hash algorithm (md5 or sha256)')
    parser.add_argument('--utv-time-offset', type=int, default=3600, help='Time offset in seconds to add for UTV mode (default: 3600)')
    parser.add_argument('--utv-time-param', default='px-time', help='Parameter name for time in UTV mode (default: px-time)')
    parser.add_argument('--utv-hash-param', default='px-hash', help='Parameter name for hash in UTV mode (default: px-hash)')
    parser.add_argument('--uid', default=None, help='Custom UID for mode E (overrides --uid-type)')
    parser.add_argument('--uid-type', choices=['mac', 'random', 'zero', 'hostname'], default='mac', help='UID generation method for mode E (default: mac)')
    parser.add_argument('--sign-order', default='pkt', help='Signing string order for modes A/B/C/D using k(ey), p(ath), t(ime) tokens (default: pkt = path+key+time, e.g. kpt = key+path+time)')
